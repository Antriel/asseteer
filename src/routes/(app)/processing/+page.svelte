...
<div class="flex flex-col h-full overflow-auto p-6">
  <div class="w-full max-w-3xl mx-auto">
    ...
    <div class="mb-6">
      ...
      <div
        class="mt-4 transition-opacity duration-200"
        class:opacity-0={!anyRunning || overallProgress.total === 0}
      >
        <div class="flex items-center justify-between mb-1.5 text-xs">
          <span class="text-secondary font-medium">Overall progress</span>
          <span class="text-primary font-medium">
            {overallProgress.completed + overallProgress.failed} / {overallProgress.total} &middot; {overallProgress.percentage}%{#if overallProgress.failed > 0}
              &middot; <span class="text-error">{overallProgress.failed} failed</span>{/if}
          </span>
        </div>
    ...
    {#if anyPaused}
      <div
        class="flex items-center gap-3 p-4 bg-warning/10 border border-warning/30 rounded-lg mt-4"
      >
        <svg
