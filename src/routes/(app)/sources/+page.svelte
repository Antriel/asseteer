<script lang="ts">
  ...
  function applyScanProgress(folderPath: string, e: ScanProgressEvent) {
    ...
    const details = {
      ...
      currentPath:
        e.current_path ?? uiState.activeScans.get(folderPath)?.details.currentPath ?? null,
    };
              ...
              {#if folder.scan_warnings}
                ...
                <div class="mt-3 rounded-md bg-warning/10 border border-warning/20 px-3 py-2">
                  <p class="text-xs font-medium text-warning mb-1">
                    {warnings.length} file{warnings.length !== 1 ? 's' : ''} could not be read during
                    last scan
                  </p>
                  ...
                  <div class="flex items-center justify-between gap-4">
                    <div class="flex items-center gap-4 text-sm">
                      ...
                      <span class="text-tertiary"
                        >{p.unchanged_count.toLocaleString()} unchanged</span
                      >
                    </div>
                    <div class="flex items-center gap-2">
                      <button
